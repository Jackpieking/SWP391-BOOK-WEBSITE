...
public class ChapterRepository : GenericRepository<ChapterEntity>, IChapterRepository
{
	...
	/// <returns></returns>
	public async Task<IEnumerable<ChapterEntity>> GetChaptersWith_ChapterId_ChapterNumber_ChapterUnlockPrice_ChapterAddedDateByComicIdAsync(Guid comicIdentifier)
	{
		return await _dbSet
	...
	/// </summary>
	/// <returns></returns>
	public async Task<IEnumerable<ChapterEntity>> GetChapterWith_ChapterNumber_ComicIdAsync()
	{
		return await _dbSet
			.Select(selector: chapterEntity => new ChapterEntity
			{
	...
	public async Task<IDictionary<Guid, string>> GetTheLastestChapterNumberOfComicsAsync()
	{
